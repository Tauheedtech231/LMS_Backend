...
export default function ModuleCard({ id, title, description, progress, estimatedTime }: Props) {
  return (
    <motion.div
      ...
      className="bg-blue-600 rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all"
    >
      <div className="p-4 flex flex-col justify-between h-full text-white">
        ...
        {/* Progress Bar */}
        <div className="mt-4 h-2 w-full bg-blue-500/50 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
        ...
        <Link href={`/student/modules/${id}`}>
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="mt-4 w-full py-2 bg-white text-blue-600 font-medium text-sm rounded-lg hover:bg-gray-100 transition-all"
          >
            Open Module
          </motion.button>
        </Link>
